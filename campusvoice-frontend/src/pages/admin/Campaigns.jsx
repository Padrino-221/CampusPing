...
import { listAllCampaigns } from '../../api/admin';
...
export default function AdminCampaigns() {
  ...
  useEffect(() => {
    listAllCampaigns(page).then(({ data }) => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="text-left p-4 text-xs font-medium text-text-muted uppercase">Candidate</th>
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                ...
                <td className="p-4 text-text-muted">{c.candidate_name || '—'}</td>
