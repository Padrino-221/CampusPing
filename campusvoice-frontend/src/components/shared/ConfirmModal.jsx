      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="card p-6 max-w-sm w-full space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="px-4 py-2 text-sm rounded-xl border border-gray-200 text-text-muted hover:bg-gray-50 cursor-pointer">Cancel</button>
            <button onClick={onConfirm} className={`px-4 py-2 text-sm rounded-xl text-white cursor-pointer ${variant === 'danger' ? 'bg-coral hover:bg-red-600' : 'bg-primary hover:bg-blue-700'}`}>{confirmLabel}</button>
          </div>
        </DialogPanel>
      </div>
    </Dialog>
